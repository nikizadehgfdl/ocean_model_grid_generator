...
def main(argv):
    ...
    if("mercator" in grids or "all" in grids):
        ...
        if(refineR == 1 and enhanced_equatorial): lat0_bp = phiMerc[-1,Ni//4]  #SPEAR
        if(match_dy):
    ...
    if("bipolar" in grids or "all" in grids):
        if(not reproduce_MIDAS_grids):
            ...
            print("   CHECK_metrics_MIDAS: % errors in (area, lat arc, lon arc)", metrics_error(dxBP,dyBP,areaBP,Ni,lat0_bp,90.))
